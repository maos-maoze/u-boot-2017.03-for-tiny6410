...
//	nfmecc0 = readl(NFMECC0);
	nfmecc[0] = readl(NFM8ECC0);
	nfmecc[1] = readl(NFM8ECC1);
	nfmecc[2] = readl(NFM8ECC2);
	nfmecc[3] = readl(NFM8ECC3);

	ecc_code[0] = (nfmecc[0] >> 0) & 0xff;
	ecc_code[1] = (nfmecc[0] >> 8) & 0xff;
	ecc_code[2] = (nfmecc[0] >> 16) & 0xff;
	ecc_code[3] = (nfmecc[0] >> 24) & 0xff;
	
	ecc_code[4] = (nfmecc[1] >> 0) & 0xff;
	ecc_code[5] = (nfmecc[1] >> 8) & 0xff;
	ecc_code[6] = (nfmecc[1] >> 16) & 0xff;
	ecc_code[7] = (nfmecc[1] >> 24) & 0xff;
	
	ecc_code[8] = (nfmecc[2] >> 0) & 0xff;
	ecc_code[9] = (nfmecc[2] >> 8) & 0xff;
	ecc_code[10] = (nfmecc[2] >> 16) & 0xff;
	ecc_code[11] = (nfmecc[2] >> 24) & 0xff;
	
	ecc_code[12] = (nfmecc[3] >> 0) & 0xff;
...
#if 1
	val = readl(NFSTAT);
	while((val & NFSTAT_ECCDECDONE) == 0)
	{
		val = readl(NFSTAT);
	}
#endif

	val = readl(NF8ECCERR0);
